...
class TaskData :
    def __init__(self):
        ...
        self._quantityTotal = None
        self._quantityRequested = None
        self._itemChoices = None
        ...
        self._account = None
        self._password = None
        self._addrDisplayStr = None
        self._addrIndex = None

class WebBitmap(wx.StaticBitmap):
    def __init__(self, parent, id, url=None,size=None):
    ...
    def Load(self,url=None):
        ...
        if self._url:
            #print "loading %s" % (self._url)
            opener = urllib2.build_opener()
            image_bytes = opener.open(self._url).read()
            data_stream = cStringIO.StringIO(image_bytes)
            self._image=wx.ImageFromStream(data_stream)
        
        if self._image:
            #adjust the size to maximize the pic in the window
            oriWidth = self._image.GetWidth()
            oriHeight = self._image.GetHeight()
            oriRatio = float(oriHeight)/float(oriWidth)
            windowRatio = float(self._size.height)/float(self._size.width)
            imgWidth = 0
            imgHeight = 0
            if oriRatio < windowRatio:
                imgWidth = self._size.width
                imgHeight = float(imgWidth)*float(oriRatio)
            else:
                imgHeight = self._size.height
                imgWidth = float(imgHeight)/oriRatio
            
            tmpImage = self._image.Copy()
            
            self.SetBitmap(tmpImage.Rescale(imgWidth,imgHeight).ConvertToBitmap())
            self.Show(True)

        return True

class TaobaoBrowser(Browser):
    def __init__(self,url=None):
        #this is the bug of sgmllib, fix it here, otherwise, cannot
        #recognize certain forms such as <br/><form>
        #the <form> will be skipped and treated as br's data
        #this fix should be global though, to avoid other potential issue
        sgmllib.shorttagopen = re.compile('<[a-zA-Z][-.a-zA-Z0-9]*/[^>]')
        sgmllib.shorttag = re.compile('<([a-zA-Z][-.a-zA-Z0-9]*)/([^/><]*)/')
        
        #init after fix, to ensure init properly
        Browser.__init__(self,url)

class TaobaoWorkerEvent:
    EVT_GET_CHECKCODE= 0
    EVT_DO_BID = 1
    EVT_QUIT = 99
    EventStr = {
        EVT_GET_CHECKCODE:'GETCC',
        EVT_DO_BID:'DOBID',
        EVT_QUIT:'QUIT'
    }
...
def DebugLog(logstr):
    ...
    now = datetime.now()
    return '[%s] %s' % (str(now),logstr)

def LoginIntoTaobao(browser,account,password):
    #now the form is possible to have logged in
    form = None
    try:
        form=browser.getForm('J_StandardForm')
    except:
        form = None
    if form:
        #print 'Login into Taobao as %s, pass[%s]' % (account,password)
        #print 'Login into Taobao as %s' % (account)
        form.getControl(name='TPL_username').value = account.encode('gbk')
        form.getControl(name='TPL_password').value = password
        form.submit()

    #check and bypass weak passwork confirmation
    form = None
    try:
        #print 'Weak password detected by taobao, double confirmation required but bypassed'
        form=browser.getForm('J_guestLoginForm')
    except:
        form=None
    if form:
        form.submit()
...
def GetAddressMap():
    ...
    accountNum = config.ReadInt("AccountNumber")
    for i in range(0,accountNum):
        account = config.Read("Account"+str(i))
        addrMap[account]=[]
        addrNum = config.ReadInt("AddressNumber"+str(i))
        for j in range(0,addrNum):
            addr0 = config.Read("AddressName"+str(i)+"_"+str(j))
            addr1 = config.Read("AddressArea"+str(i)+"_"+str(j))
            addr2 = config.Read("AddressFull"+str(i)+"_"+str(j))
            addr3 = config.Read("AddressPost"+str(i)+"_"+str(j))
            addr4 = config.Read("AddressPhone"+str(i)+"_"+str(j))
            addr5 = config.Read("AddressId"+str(i)+"_"+str(j))
            #addrMap[account].append([addr0,addr1,addr2,addr3])
            addrMap[account].append(
                {
                    'AddressName':addr0,
                    'AddressArea':addr1,
                    'AddressFull':addr2,
...
def StringToDatetime(str):
    ...
    #print str
    try:
        p=re.compile('(\d\d\d\d)[^d]+(\d\d)[^d]+(\d\d)[^d]+ (\d\d)[^d]+(\d\d)[^d]+(\d\d)[^d]+')
        r=p.findall(str)
        if len(r)>0:
            time = datetime.datetime(atoi(r[0][0]),atoi(r[0][1]),atoi(r[0][2]),atoi(r[0][3]),atoi(r[0][4]),atoi(r[0][5]))
        #print r
        #print time
    except:
        pass
...
def LocateAreaCode(addr):
    #addr = '广东省 珠海市 斗门区'
    #addr is already gbk-encoded
    import string
    #print addr
    #print addr.encode('gbk')
    [prov,city,area]=addr.split(' ')
    prov1 = string.join(['\u%04x'%ord(c) for c in prov],'')
    city1 = string.join(['\u%04x'%ord(c) for c in city],'')
    area1 = string.join(['\u%04x'%ord(c) for c in area],'')
    #print prov1
    #print city1
    #print area1
    provCode = 0
    cityCode = 0
    areaCode = 0
    for code in AreaCodeMap:
        if AreaCodeMap[code][0] == prov1:
            provCode = code
    #print provCode

    for code in AreaCodeMap:
        if AreaCodeMap[code][0] == city1 and AreaCodeMap[code][1] == provCode:
            cityCode = code
    #print cityCode

    for code in AreaCodeMap:
        if AreaCodeMap[code][0] == area1 and AreaCodeMap[code][1] == cityCode:
            areaCode = code
    #print areaCode
